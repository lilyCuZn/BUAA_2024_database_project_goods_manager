  ...
  <div>
    ...
    >
      ...
      <div>
        <span>
          第 {{ currentPage }} 页，总计
          {{ storageApproves.length }} 条，{{ totalPages }}
          页
        </span>
      </div>
    </div>
...
export default {
  ...
  async mounted() {
    console.log("mounted");
    console.log("getStorageApproves");
    await this.updateStorageApproves();
  },
  data() {
    return {
      storageApproves: [],
  ...
  computed: {
    paginatedData() {
      ...
      return this.storageApproves.slice(start, end);
    },
    totalPages() {
      return Math.ceil(
        this.storageApproves.length / this.itemsPerPage
      );
    },
  },
  methods: {
    ...
    async updateStorageApproves() {
      console.log("updateStorageApproves");
      this.isLoading = true;
      let req = {
        action: "getStorageApproves",
      };
      console.log("req:", req);
      let msg = await this.$Backend(req);
      if (msg && msg.result === "success") {
        console.log("updateStorageApproves success");
        this.storageApproves = msg.applications;
      }
    ...
    exportData() {
      console.log("exportstorageApproves");
      const modifiedData = this.storageApproves.map(
        (item) => {
          return {
            申请编号: item.id,
